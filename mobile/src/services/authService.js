...
const authService = {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      // Clear stale shop cache from any previous session/user so this
      // login gets a fresh, correctly-filtered shop list from the server
      const shopService = require('./shopService').default;
      ...
      if (user.role_id === 3 && user.salesman_id) {
        try {
          ...
          await syncService.syncSalesmen(true);
